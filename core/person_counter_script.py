...
def detect(frame, detections, trackers, rects):
...
def track(frame, trackers, rects):
...
def counting(frame, objects, trackableObjects):  # , totalIn, totalOut):
    global totalIn
    global totalOut
    ...
    while True:
        ...
        if vs is not None:
            ...
            if elapsedFrames % skipFrames == 0:
                ...
                detect(frame, detections, trackers, rects)

            # do tracking using detection data on every other frame
            else:
                track(frame, trackers, rects)

            objects = ct.update(rects)
            counting(frame, objects, trackableObjects)  # , totalIn, totalOut)
